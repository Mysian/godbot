...
async function findMember(guild, raw){
  ...
  const qs=tokens(raw).filter(t=>t.length>=2);
...
function parseApproval(text){
  ...
  out.userIdFromMention = pick(/<@!?(\d{15,25})>/, text);
  out.userIdFromTarget  = pick(/대상유저\s*=\s*.*?\((\d{15,25})\)/, text);

  out.nickname   = pick(/닉네임\s*[:=]\s*([^\n]+)/, text);
  out.birthYear  = pick(/출생년도\s*[:=]\s*([0-9]{4})/, text);
  out.gender     = pick(/성별\s*[:=]\s*([^\n]+)/, text);
  out.referrer   = pick(/추천인\s*[:=]\s*([^\n]+)/, text);
  out.ingress    = pick(/(가입 경로|유입경로)\s*[:=]\s*([^\n]+)/, text) || null;
  out.playstyle  = pick(/플레이스타일\s*[:=]\s*([^\n]+)/, text);

  out.gameChosen = pick(/선택\s*=\s*([^\n]+)/, text);
  out.gameRoles  = pick(/부여된역할\s*=\s*([^\n]+)/, text);
  out.alertTags  = pick(/알림 태그[\s\S]*?설정\s*=\s*([^\n]+)/, text);

  out.log_time   = pick(/시간\s*=\s*([^\n]+)/, text);
  out.log_actor  = pick(/처리자\s*=\s*([^\n]+)/, text);
  out.log_target = pick(/대상유저\s*=\s*([^\n]+)/, text);
  out.log_silent = pick(/조용히승인\s*=\s*([^\n]+)/, text);
  out.log_alt    = pick(/부계정여부\s*=\s*([^\n]+)/, text);

  out.hist_reject = pick(/거절\s*=\s*([0-9]+회)/, text);
  out.hist_join   = pick(/총입장\s*=\s*([0-9]+회)/, text);
  out.hist_leave  = pick(/총퇴장\s*=\s*([0-9]+회)/, text);
  out.hist_click  = pick(/클릭(?:\(게임장\))?\s*=\s*([0-9]+회)/, text);
...
async function scanSource(client,guildId,sourceChannelId,uid,member){
  ...
  if(!ch||ch.guildId!==guildId) return {record:null, reason:'NO_ACCESS'};
  ...
  for(let i=0;i<15;i++){
    ...
    for(const [,m] of msgs){
      ...
      if(matchRecordToUser(parsed, uid, member)){
        if(!latest||m.createdTimestamp>latest.createdTimestamp){
          latest={message:m,text:blob};
          latestParsed=parsed;
        }
      }
    }
  ...
  return {record:latest, parsed:latestParsed, reason: latest? 'OK':'NOT_FOUND'};
}
...
function enrichWithParsed(eb, parsed){
  if(!parsed) return eb;
  const prof=[];
  if(parsed.nickname) prof.push(`닉네임: ${parsed.nickname}`);
  if(parsed.birthYear) prof.push(`출생년도: ${parsed.birthYear}`);
  if(parsed.gender) prof.push(`성별: ${parsed.gender}`);
  if(parsed.ingress) prof.push(`유입경로: ${parsed.ingress}`);
  if(parsed.referrer) prof.push(`추천인: ${parsed.referrer}`);
  if(parsed.playstyle) prof.push(`플레이스타일: ${parsed.playstyle}`);
  if(prof.length) eb.addFields({name:'프로필',value:prof.join('\n'),inline:false});

  const game=[];
  if(parsed.gameChosen) game.push(`선택: ${parsed.gameChosen}`);
  if(parsed.gameRoles) game.push(`부여된역할: ${parsed.gameRoles}`);
  if(game.length) eb.addFields({name:'게임 태그',value:game.join('\n'),inline:false});

  if(parsed.alertTags) eb.addFields({name:'알림 태그',value:`설정: ${parsed.alertTags}`,inline:false});

  const hist=[];
  if(parsed.hist_reject) hist.push(`거절: ${parsed.hist_reject}`);
  if(parsed.hist_join)   hist.push(`총입장: ${parsed.hist_join}`);
  if(parsed.hist_leave)  hist.push(`총퇴장: ${parsed.hist_leave}`);
  if(parsed.hist_click)  hist.push(`클릭: ${parsed.hist_click}`);
  if(hist.length) eb.addFields({name:'이력 요약',value:hist.join('\n'),inline:false});

  const log=[];
  if(parsed.log_time)  log.push(`시간: ${parsed.log_time}`);
  if(parsed.log_actor) log.push(`처리자: ${parsed.log_actor}`);
  if(parsed.log_target)log.push(`대상유저: ${parsed.log_target}`);
  if(parsed.log_alt)   log.push(`부계정여부: ${parsed.log_alt}`);
  if(parsed.log_silent)log.push(`조용히승인: ${parsed.log_silent}`);
  if(log.length) eb.addFields({name:'입장 승인 로그',value:log.join('\n'),inline:false});

  return eb;
}

function addRecordPreview(eb, record){
  const preview=record?(record.text.length>1024?record.text.slice(0,1021)+'…':record.text):'해당 채널에서 기록을 찾지 못함';
  eb.addFields({name:'기록 채널 최근 기록',value:preview,inline:false});
}

function addLastMessage(eb, guild, msg){
  const value = msg
    ? `채널: <#${msg.channelId}>\n시간: <t:${Math.floor(msg.createdTimestamp/1000)}:F>\n내용: ${msg.content&&trim(msg.content)!==''?(msg.content.length>900?msg.content.slice(0,897)+'…':msg.content):(msg.embeds?.length?'(임베드/첨부 메시지)':'(내용 없음)')}\n링크: https://discord.com/channels/${guild.id}/${msg.channelId}/${msg.id}`
    : '최근 메시지 없음';
  eb.addFields({name:'가장 최근 메시지',value,inline:false});
}
...
function registerUserInfoLookup(client,{sourceChannelId,triggerChannelId}){
  ...
  client.on('messageCreate',async(message)=>{
    try{
      ...
      const [{record,parsed,reason}, lastMsg]=await Promise.all([
      ...
      if(parsed) eb=enrichWithParsed(eb,parsed);
      addRecordPreview(eb, record);
      addLastMessage(eb, message.guild, lastMsg);

      if(reason==='NO_ACCESS'){
        eb.setFooter({text:'기록 채널 접근 권한이 없어 원문 스캔을 건너뜀'});
      }

      await message.channel.send({embeds:[eb]});
    }catch(e){
      try{await message.reply('조회 중 오류가 났어. 봇 권한(메시지 보기/기록 열람)과 채널 ID 설정을 확인해줘.');}catch{}
    }
  });
}
