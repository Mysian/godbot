...
const { setGlobalDispatcher, ProxyAgent, Agent: UndiciAgent } = require('undici');
...
try {
  if (process.env.YT_PROXY) {
    ...
    undiciProxy = new ProxyAgent(process.env.YT_PROXY);
    setGlobalDispatcher(undiciProxy);
    process.env.HTTPS_PROXY = process.env.HTTPS_PROXY || process.env.YT_PROXY;
    process.env.HTTP_PROXY = process.env.HTTP_PROXY || process.env.YT_PROXY;
    console.log('[music] proxy enabled:', process.env.YT_PROXY);
  } else {
    setGlobalDispatcher(new UndiciAgent({ keepAliveTimeout: 15_000, keepAliveMaxTimeout: 30_000 }));
  }
} catch {}

try {
  if (process.env.YT_FORCE_IPV4 === '1') dns.setDefaultResultOrder('ipv4first');
} catch {}
...
if (process.env.YT_COOKIE) {
  try { play.setToken({ youtube: { cookie: process.env.YT_COOKIE, gl: 'KR', hl: 'ko' } }); console.log('[music] YT_COOKIE set for play-dl'); } catch {}
}
...
function makeYtdl(url, { dlChunkSize = 0, itags = [140, 251, 250, 249] } = {}) {
  ...
  if (process.env.YT_FORCE_IPV4 === '1') {
    requestOptions.lookup = (hostname, opts, cb) => dns.lookup(hostname, { family: 4 }, cb);
  }
  return ytdl(url, {
    filter: 'audioonly',
    quality: itags,
...
async function playIndex(guildId, client) {
  ...
  try {
    ...
    else if (/ENOTFOUND/i.test(raw)) causeForUser = 'DNS/프록시 호스트 해석 실패';
