...
function ensureRecord(store, uid) {
  if (!store.users[uid]) store.users[uid] = { status: null, countJoinAttempts: 0, countApproved: 0, countRejected: 0, rejectedBirthYears: [], history: [], activeChannelId: null, flow: null, pendingDecisionMessageId: null, lastStepMsgIds: {} };
  if (!store.users[uid].lastStepMsgIds) store.users[uid].lastStepMsgIds = {};
...
function gateEmbed() {
  return new EmbedBuilder()
    ...
    .setColor(0x7b2ff2)
    .setDescription(["아래 버튼으로 입장 절차를 시작하세요.", "신규/재입장/부계 확인 후 관리진 승인으로 마무리됩니다.", `입장 가능 출생년도: **${minAllowedBirthYear()}년 이하**`].join("\n"));
}
...
async function postGateIfMissing(guild) {
  try {
    ...
    const exists = msgs?.some(m => m.author?.bot && m.components?.[0]?.components?.[0]?.customId === "gate_start");
    if (!exists) await ch.send({ embeds: [gateEmbed()], components: [gateRow()] });
  } catch {}
}

function rowType() {
  return new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId("type_new").setStyle(ButtonStyle.Primary).setLabel("신규 입장"),
    new ButtonBuilder().setCustomId("type_rejoin").setStyle(ButtonStyle.Secondary).setLabel("재입장"),
    new ButtonBuilder().setCustomId("type_alt").setStyle(ButtonStyle.Success).setLabel("부계정 생성")
  );
}
function rowGender() {
  return new ActionRowBuilder().addComponents(
...
function rowTags(todo) {
  return new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId("open_select_settings").setStyle(ButtonStyle.Primary).setLabel(todo?.settingsDone ? "서버 태그 ✔" : "서버 태그 설정"),
    new ButtonBuilder().setCustomId("done_settings").setStyle(todo?.settingsDone ? ButtonStyle.Secondary : ButtonStyle.Success).setLabel(todo?.settingsDone ? "서버 태그 완료됨" : "서버 태그 완료")
  );
}
function rowTags2(todo) {
  return new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId("open_select_games").setStyle(ButtonStyle.Primary).setLabel(todo?.gamesDone ? "게임 태그 ✔" : "게임 태그 설정"),
    new ButtonBuilder().setCustomId("done_games").setStyle(todo?.gamesDone ? ButtonStyle.Secondary : ButtonStyle.Success).setLabel(todo?.gamesDone ? "게임 태그 완료됨" : "게임 태그 완료")
  );
}
function rowNick() {
  return new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId("open_nick").setStyle(ButtonStyle.Primary).setLabel("별명 입력"));
}
function rowDecision(ctxId) {
  return new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId(`approve_user:${ctxId}`).setStyle(ButtonStyle.Success).setLabel("승인"),
    new ButtonBuilder().setCustomId(`approve_silent:${ctxId}`).setStyle(ButtonStyle.Primary).setLabel("조용히 승인"),
...
async function disableMessageButtons(channel, messageId) {
  ...
  try {
    ...
    const comps = msg.components.map(row => {
      ...
      r.addComponents(...row.components.map(c => ButtonBuilder.from(c).setDisabled(true)));
      return r;
    });
    await msg.edit({ components: comps }).catch(() => {});
  } catch {}
}

function embedStep(title, desc, color) { return new EmbedBuilder().setTitle(title).setDescription(desc).setColor(color || 0x95a5a6); }

async function openModalBirth(i) {
  const modal = new ModalBuilder().setCustomId("modal_birth").setTitle("출생년도 입력");
  const t = new TextInputBuilder().setCustomId("birth").setLabel("출생년도 (예: 2005)").setStyle(TextInputStyle.Short).setRequired(true).setMaxLength(4);
  modal.addComponents(new ActionRowBuilder().addComponents(t));
  await i.showModal(modal);
}
async function openModalNick(i) {
  const modal = new ModalBuilder().setCustomId("modal_nick").setTitle("서버 별명 입력");
  const t = new TextInputBuilder().setCustomId("nick").setLabel("서버에서 사용할 별명").setStyle(TextInputStyle.Short).setRequired(true).setMaxLength(32);
  modal.addComponents(new ActionRowBuilder().addComponents(t));
  await i.showModal(modal);
}
async function openModalRef(i) {
  const modal = new ModalBuilder().setCustomId("modal_ref").setTitle("추천인 닉네임 입력");
  const t = new TextInputBuilder().setCustomId("ref").setLabel("추천인 닉네임").setStyle(TextInputStyle.Short).setRequired(true).setMaxLength(32);
  modal.addComponents(new ActionRowBuilder().addComponents(t));
  await i.showModal(modal);
}
async function openModalAlt(i) {
...
function buildSummaryEmbed(ctx) {
  const e = new EmbedBuilder()
    ...
    .addFields(
      ...
      { name: "입장 경로", value: ctx.type === "부계정" ? `본계정: ${ctx.mainNickname}` : `${ctx.source || "-"}${ctx.referrer ? ` / 추천인: ${ctx.referrer}` : ""}`, inline: false },
  ...
  const e = new EmbedBuilder()
    ...
    .setDescription(
      [
        ...
        `• 유형: ${ctx.type}`,
        ctx.type === "부계정" ? `• 본계정: ${ctx.mainNickname} / 출생년도: ${ctx.mainBirthYear}` : `• 출생년도: ${ctx.birthYear} / 성별: ${ctx.gender} / 경로: ${ctx.source}${ctx.referrer ? ` / 추천인: ${ctx.referrer}` : ""}`,
        `• 별명: ${ctx.nickname || "-"}`,
...
function buildRecordSummary(rec) {
  const arr = [];
  arr.push(`• 누적 시도: ${rec.countJoinAttempts}회`);
  arr.push(`• 승인: ${rec.countApproved}회, 거절: ${rec.countRejected}회`);
  if (rec.rejectedBirthYears?.length) arr.push(`• 출생년도 오입력 거절: ${rec.rejectedBirthYears.join(", ")}`);
  return arr.join("\n");
}
...
async function beginFlow(i) {
  ...
  const intro = await ch.send({ embeds: [new EmbedBuilder().setTitle(`👋 ${i.member.displayName}님 환영!`).setDescription([`아래에서 유형을 선택해줘.`, buildRecordSummary(rec)].join("\n")).setColor(0x2095ff)], components: [rowType()] });
  const s = await loadStore(); const r = ensureRecord(s, i.user.id); r.lastStepMsgIds.type = intro.id; await saveStore(s);
  return ch;
}
...
async function clearFlowAndChannel(guild, uid) {
  ...
  r.flow = null; if (r.pendingDecisionMessageId && s.messages[r.pendingDecisionMessageId]) delete s.messages[r.pendingDecisionMessageId]; r.pendingDecisionMessageId = null; await saveStore(s);
}
...
async function handleDecision(i, action, ctxFromId) {
  ...
  if (!saved || (ctxFromId && saved.ctxId !== ctxFromId)) {
    await i.reply({ content: "컨텍스트가 만료되었습니다.", ephemeral: true });
    return;
  }
  const guild = i.guild;
  ...
  if (action === "APPROVE" || action === "APPROVE_SILENT") {
    ...
    if (ctx.nickname) {
      ...
      if (ok) { try { await member.setNickname(ctx.nickname).catch(() => {}); } catch {} }
    }
    ...
    try {
      if (rec.activeChannelId) {
        const ch = await guild.channels.fetch(rec.activeChannelId).catch(() => null);
        if (ch) await ch.send({ content: `<@${member.id}> 승인 심사에서 거절되었습니다. 문의는 운영진에게 부탁해.` }).catch(() => {});
      }
    } catch {}
    try { await clearFlowAndChannel(guild, member.id); } catch {}
  }
...
async function handleAltFinalize(i, values) {
  ...
  const msgId = await sendDecisionCard(guild, ctx, rec);
  await i.reply({ content: "부계정 심사 요청이 접수되었습니다. 관리진 승인을 기다려줘.", ephemeral: true });
  if (rec.activeChannelId) {
    const ch = await guild.channels.fetch(rec.activeChannelId).catch(() => null);
    if (ch) {
      const waitMsg = await ch.send({ embeds: [embedStep("대기 안내", "관리진의 승인을 기다려줘.", 0x95a5a6)] });
      rec.lastStepMsgIds.wait = waitMsg.id; await saveStore(store);
    }
  }
}

async function handleBirthValidation(yearStr) {
...
function top5GamesFromFlow(flow) {
  if (!flow || !flow.selectedGames || !Array.isArray(flow.selectedGames)) return [];
  return flow.selectedGames.slice(0, 5);
}

function buildDecisionCtxFromFlow(flow, member) {
  return {
    type: flow.type || "신규/재입장",
    birthYear: flow.birthYear,
    gender: flow.gender,
    source: flow.source,
    referrer: flow.referrer,
    nickname: flow.nickname,
    tagsDone: !!(flow.settingsDone && flow.gamesDone),
    member,
    mainNickname: flow.mainNickname,
    mainBirthYear: flow.mainBirthYear,
...
async function collectFlow(client) {
  ...
  client.on("guildMemberRemove", async member => {
    try {
      const store = await loadStore(); const rec = ensureRecord(store, member.id);
      await clearFlowAndChannel(member.guild, member.id);
      rec.status = null; await saveStore(store);
    } catch {}
  });
