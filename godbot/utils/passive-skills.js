...
"나서스": {
  ...
  passive: (user, enemy, context) => {   context.effects[enemy.id] = context.effects[enemy.id] || [];   context.effects[user.id] = context.effects[user.id] || [];
    ...
    if (
      ["defend", "dodge", "item"].includes(context.lastAction) &&
      user._nasusStacks > 0 &&
      Math.random() < 0.5
    ) {
      user._nasusStacks = 0;
...
"드레이븐": {
  ...
  passive: (user, enemy, context) => {   context.effects[enemy.id] = context.effects[enemy.id] || [];   context.effects[user.id] = context.effects[user.id] || [];
    ...
    if (["defend", "dodge", "item", "stunned", "skip"].includes(context.lastAction)) {
...
"멜": {
  ...
  passive: (user, enemy, context) => {   context.effects[enemy.id] = context.effects[enemy.id] || [];   context.effects[user.id] = context.effects[user.id] || [];
    ...
    if (context.lastAction === "defend" && Math.random() < 0.10 && context.damage > 0) {
...
"브랜드": {
  ...
  passive: (user, enemy, context) => {   context.effects[enemy.id] = context.effects[enemy.id] || [];   context.effects[user.id] = context.effects[user.id] || [];
    ...
    if (dotCount > 0 && context.lastAction === "attack") {
...
"사이온": {
  ...
  passive: (user, enemy, context) => {   context.effects[enemy.id] = context.effects[enemy.id] || [];   context.effects[user.id] = context.effects[user.id] || [];
    ...
    if (user._sionReviveUsed && user._sionReviveTurns !== undefined && context.lastAction === "turnStart") {
...
"샤코": {
  ...
  passive: (user, enemy, context) => {   context.effects[enemy.id] = context.effects[enemy.id] || [];   context.effects[user.id] = context.effects[user.id] || [];
    ...
    if (user._shacoDodgeTurns > 0 && context.lastAction === "turnStart") {
...
"세트": {
  ...
  passive: (user, enemy, context) => {   context.effects[enemy.id] = context.effects[enemy.id] || [];   context.effects[user.id] = context.effects[user.id] || [];
    ...
    // 다음 턴 처리 (turnStart 시)
    if (context.lastAction === "turnStart" && user._setHealEnemyNextTurn) {
...
"쉬바나": {
  ...
  passive: (user, enemy, context) => {   context.effects[enemy.id] = context.effects[enemy.id] || [];   context.effects[user.id] = context.effects[user.id] || [];
    ...
    if (context.lastAction === "defend" && Math.random() < 0.5) {
...
"시비르": {
  ...
  passive: (user, enemy, context) => {   context.effects[enemy.id] = context.effects[enemy.id] || [];   context.effects[user.id] = context.effects[user.id] || [];
    ...
    // 방어 시 상대 스킬 피해 완전 무효
    if (context.lastAction === "defend" && context.isSkillAttack) {
      context.damage = 0;
...
"쓰레쉬": {
  ...
  passive: (user, enemy, context) => {   context.effects[enemy.id] = context.effects[enemy.id] || [];   context.effects[user.id] = context.effects[user.id] || [];
    ...
    if (context.lastAction === "attack") {
      ...
      if (chance > 0.10) chance = 0.30; // 최대 10% 제한
      if (Math.random() < chance) {
...
"아이번": {
  ...
  passive: (user, enemy, context) => {   context.effects[enemy.id] = context.effects[enemy.id] || [];   context.effects[user.id] = context.effects[user.id] || [];
    ...
    if (user._ivernShieldTurns > 0 && context.lastAction === "defend" && context.damage > 0) {
      context.damage = 0;
    ...
    if (user._ivernShieldTurns === 0 && user._ivernPostShieldTurns < 10 && context.lastAction === "turnStart") {
...
"애니비아": {
  ...
  passive: (user, enemy, context) => {   context.effects[enemy.id] = context.effects[enemy.id] || [];   context.effects[user.id] = context.effects[user.id] || [];
    ...
    if (user._aniviaAfterRevive && context.lastAction === "defend" && context.damage > 0) {
...
"이즈리얼": {
  ...
  passive: (user, enemy, context) => {   context.effects[enemy.id] = context.effects[enemy.id] || [];   context.effects[user.id] = context.effects[user.id] || [];
    ...
    if ((context.lastAction === "defend" || context.lastAction === "dodge") && user._ezrealCritChance > 0.1) {
...
"일라오이": {
  ...
  passive: (user, enemy, context) => {   context.effects[enemy.id] = context.effects[enemy.id] || [];   context.effects[user.id] = context.effects[user.id] || [];
    ...
    // 피해량 보정은 항상 적용
    if (user._illaoiDmgBonus && context.lastAction === "attack") {
...
"카사딘": {
  ...
  passive: (user, enemy, context) => {   context.effects[enemy.id] = context.effects[enemy.id] || [];   context.effects[user.id] = context.effects[user.id] || [];
    ...
    // 10턴까지 디버프 면역
    if (user._kassadinTurn <= 10 && context.lastAction === "turnStart") {
      context.effects[user.id].push({ type: "debuffImmune", turns: 1 });
      return "🛡️ 모든 디버프 면역!";
    }

    // 10턴 이후, 5턴마다 디버프 해제
    if (user._kassadinTurn > 10 && context.lastAction === "turnStart" && (user._kassadinTurn - 10) % 5 === 0) {
...
"카서스": {
  ...
  passive: (user, enemy, context) => {   context.effects[enemy.id] = context.effects[enemy.id] || [];   context.effects[user.id] = context.effects[user.id] || [];
    ...
    if (user._karthusUndyingTurns && user._karthusUndyingTurns > 0 && context.lastAction === "attack") {
      context.damage = Math.floor(context.damage * 1.5);
...
"카시오페아": {
  ...
  passive: (user, enemy, context) => {   context.effects[enemy.id] = context.effects[enemy.id] || [];   context.effects[user.id] = context.effects[user.id] || [];
    ...
    if (context.lastAction === "attack" && Math.random() < chance) {
      context.effects[enemy.id].push({ type: "stunned", turns: 2 });
      user._cassioPopped += 1;
      return `🐍 2턴간 기절! (현재 확률 ${(chance * 100).toFixed(1)}%)`;
    }
    // 기절 상태의 상대에게 30% 추가 피해
    if (context.lastAction === "attack" && enemy.stunned) {
      context.damage = Math.floor(context.damage * 1.3);
...
"케인": {
  ...
  passive: (user, enemy, context) => {   context.effects[enemy.id] = context.effects[enemy.id] || [];   context.effects[user.id] = context.effects[user.id] || [];
    ...
    if (enemy._lastDisabled && context.lastAction === "attack") {
      enemy._kaynDisableTurns++;
    }
    // 피해 증가량 계산(최대 50%)
    let bonus = Math.min(enemy._kaynDisableTurns * 0.05, 0.5);
    if (bonus > 0 && context.lastAction === "attack") {
      context.damage = Math.floor(context.damage * (1 + bonus));
...
"크산테": {
  ...
  passive: (user, enemy, context) => {   context.effects[enemy.id] = context.effects[enemy.id] || [];   context.effects[user.id] = context.effects[user.id] || [];
    ...
    // 피해 증가(공격)
    if (context.lastAction === "attack" && user._ksanteLastReduction && user._ksanteLastReduction > 0) {
...
"키아나": {
  ...
  passive: (user, enemy, context) => {   context.effects[enemy.id] = context.effects[enemy.id] || [];   context.effects[user.id] = context.effects[user.id] || [];
    ...
    else if (context.lastAction === "item") chance = 0.50;
...
"탈리야": {
  ...
  passive: (user, enemy, context) => {   context.effects[enemy.id] = context.effects[enemy.id] || [];   context.effects[user.id] = context.effects[user.id] || [];
    ...
    // 기절된 상대에게 20% 추가 피해
    if (user._taliyaStun && context.lastAction === "attack" && enemy.stunned) {
      context.damage = Math.floor(context.damage * 1.2);
      user._taliyaStun = false;
      return "🌋 기절 상대 추가 피해 20%!";
    }
    // 리스크: 자신이 받는 스킬 피해 40% 증가
    if (context.lastAction === "defend" && context.isSkill) {
      context.damage = Math.floor(context.damage * 1.4);
...
"피즈": {
  ...
  passive: (user, enemy, context) => {   context.effects[enemy.id] = context.effects[enemy.id] || [];   context.effects[user.id] = context.effects[user.id] || [];
    ...
    if (context.lastAction === "dodge") { // 점멸(회피) 시에만
...
"나르": {
  ...
  passive: (user, enemy, context) => {   context.effects[enemy.id] = context.effects[enemy.id] || [];   context.effects[user.id] = context.effects[user.id] || [];
    ...
    // 피해를 받을 때 카운트
    if (context.lastAction === "defend" && context.damage > 0 && !user._gnarTransformed) {
    ...
    // 변신 이후, 가하는 피해 30% 증가
    if (user._gnarTransformed && context.lastAction === "attack") {
      context.damage = Math.floor(context.damage * 1.3);
      return "🐻 변신 상태! 가하는 피해 30% 증가!";
    }

    // 변신 이후, 받는 피해 10% 증가
    if (user._gnarTransformed && context.lastAction === "defend" && context.damage > 0) {
      context.damage = Math.floor(context.damage * 1.1);
