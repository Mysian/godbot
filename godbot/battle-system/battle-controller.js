...
async function updateBattleView(interaction, battle, activeUserId) {
  await updateBattleTimer(battle, interaction, true); // force:true로 타이머 중복 응답 방지
  try {
...
// battle 버튼 핸들러 (아래는 pass 상호작용 이중응답 방지 패치 적용)
async function handleBattleButton(interaction) {
  ...
  try {
    ...
    // ↓↓↓ 실제 배틀의 모든 버튼(공격/방어/점멸/아이템/스킬/도망/pass 등) 분기 ↓↓↓
    ...
    if (action === 'item') {
      ...
      await updateBattleTimer(battle, interaction, true); // force:true
      replied = true; return;
    }
