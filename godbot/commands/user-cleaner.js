...
async function fetchAllThreadsOfChannel(channel) {
  const threads = [];
  try {
    if (channel.threads && channel.threads.fetchActive) {
      const active = await channel.threads.fetchActive();
      active?.threads?.forEach((t) => threads.push(t));
    }
  } catch {}
  try {
    if (channel.threads && channel.threads.fetchArchived) {
      let done = false;
      let before;
      while (!done) {
        const archived = await channel.threads.fetchArchived({ before, limit: 100 });
        archived?.threads?.forEach((t) => threads.push(t));
        if (!archived || archived.threads.size < 100) done = true;
        else before = archived.threads.last()?.id;
      }
    }
  } catch {}
  return threads;
...
async function scanChannelForTargets(channel, targetIdsSet, cutoffMs, includeTagged, onProgress, abortSignal) {
  ...
  async function scanOneMessageContainer(container) {
    let fetched = 0;
    let before;
    while (true) {
      ...
      for (const msg of messages) {
        ...
        before = msg.id;
        fetched += 1;
        if (cutoffMs && msg.createdTimestamp && msg.createdTimestamp < cutoffMs) {
          return;
        }
        if (targetIdsSet.has(msg.author?.id)) {
          ...
          let mentioned = false;
          for (const tid of targetIdsSet) {
            if (msg.mentions?.users?.has(tid)) {
              mentioned = true;
              break;
            }
            if (msg.content && (msg.content.includes(`<@${tid}>`) || msg.content.includes(`<@!${tid}>`))) {
              mentioned = true;
              break;
            }
          }
  ...
  if (
    ...
    await scanOneMessageContainer(channel);
    const threads = await fetchAllThreadsOfChannel(channel);
    for (const th of threads) {
      if (abortSignal?.aborted) break;
      await scanOneMessageContainer(th);
    }
  } else if (
...
function deepCleanJson(value, targetIdsSet) {
  let removed = 0;
  let changed = false;
  function clean(v, ctxParent, ctxKey) {
    ...
    if (Array.isArray(v)) {
      const nv = [];
      for (const item of v) {
        const cleaned = clean(item, v, null);
  ...
  const cleaned = clean(value, null, null);
...
function buildPlanEmbed(session) {
  const e = new EmbedBuilder()
    .setTitle("유저청소 · 계획")
    .setColor(0x8e44ad)
    .setDescription("아래 내역은 스캔/삭제 계획이며, [삭제 확정] 버튼을 누르기 전에는 실제로 삭제되지 않아.")
    .addFields(
...
async function runScan(interaction, session) {
  ...
  await interaction.editReply({
    embeds: [loadingEmbed("유저청소 · 스캔중", "채널을 스캔하고 있어.", 0, totalChannels, [
  ...
  for (const ch of channels) {
    ...
    const res = await scanChannelForTargets(
      ...
      async () => {
        if (throttle(session)) {
          await interaction.editReply({
            embeds: [loadingEmbed("유저청소 · 스캔중", "채널을 스캔하고 있어.", chIndex - 1, totalChannels, [
    ...
    await interaction.editReply({
      embeds: [loadingEmbed("유저청소 · 스캔중", "채널을 스캔하고 있어.", chIndex, totalChannels, [
        `발견: 본인메시지 ${fmtInt(session.stats.own)} · 태그됨 ${fmtInt(session.stats.tagged)}`
      ])],
      components: buildButtons(session, "plan")
    }).catch(() => {});
...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addBooleanOption((opt) =>
      opt.setName("태그삭제").setDescription("대상이 태그된 메시지도 삭제 (기본 ON)")
    )
    .addBooleanOption((opt) =>
  ...
  async component(interaction) {
    ...
    if (action === "scan") {
      ...
      const embed = buildPlanEmbed(session);
