...
async function simulateMarket(interaction, coins) {
  ...
  // 2) 기타 코인 (까리코인 변동 영향 크게 반영)
  for (const [name, info] of Object.entries(coins)) {
    if (name === '까리코인' || info.delistedAt) continue;
    // 동조화: 까리코인 변동분의 40~60% 정도 랜덤하게 추가 영향!
    const kImpact = deltaBase * (0.4 + Math.random()*0.2); // 0.4~0.6배 영향
...
// 과부하 방지: 히스토리 제한/폐지된 코인 7일 후 삭제
setInterval(async () => {
  const coins = await loadJson(coinsPath, {});
  const now = Date.now();

  for (const [name, info] of Object.entries(coins)) {
    while (info.history && info.history.length > 100)
      info.history.shift();
    ...
    collector.on('collect', async btn => {
      ...
      if (btn.customId === 'buy' || btn.customId === 'sell') {
        ...
        if (isBuy) {
          ...
          const fee = Math.floor(total * 0.3); // **매수 수수료 30%**
