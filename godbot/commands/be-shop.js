  ...
  async execute(interaction) {
    try {
        ...
        collector.on('collect', async i => {
          ...
          if (i.customId.startsWith("item_buy_")) {
            ...
            try {
              ...
              if (userBeNow < item.price) {
                await i.reply({ content: `파랑 정수 부족! (보유: ${numFmt(userBeNow)} BE)`, ephemeral: true });
                return;
              }
      ...
      if (kind === 'nickname') {
        ...
        const hexToImgUrl = (hex) => `https://singlecolorimage.com/get/${hex.replace('#', '')}/100x100`;
        const getEmbedAndRows = (_page, curBe) => {
        ...
        collector.on('collect', async i => {
          ...
          if (i.customId === 'channel_buy') {
            ...
            try {
              const be = await loadJson(bePath);
              const userBeNow = be[i.user.id]?.amount || 0;
              if (userBeNow < CHANNEL_ROLE_PRICE) {
                await i.reply({ content: `파랑 정수 부족! (보유: ${numFmt(userBeNow)} BE)`, ephemeral: true });
                return;
              }
...
> ${owned ? '**[보유중]**' : ''}`;
          ...
          if (showTitles[0]?.color && showTitles[0].color.startsWith('http')) embed.setImage(showTitles[0].color);
