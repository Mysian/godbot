...
// 외부 훅: configure / snapshot / claim / onCatch / onSell / onOpenChest / onSpend / onBaitUse / onDurability
...
function ensureUser(u){ u.quests2 ??= { dailyKey:null, weeklyKey:null, state:{} }; }
function resetKeysIfNeeded(u, dKey, wKey){ ensureUser(u); if (u.quests2.dailyKey !== dKey || u.quests2.weeklyKey !== wKey){ u.quests2 = { dailyKey:dKey, weeklyKey:wKey, state:{} }; } }
...
function makeDailyDefs(dKey){
  ...
  const candidates = [
    ...
    { type:"catch_timeband", band:rpick(rng,["낮","노을","밤"]), amount:rint(rng,5,8) },
    { type:"catch_each_timeband", perBand:rint(rng,2,3) },
    { type:"catch_specific", species:sp, amount:rint(rng,1,2) },
    { type:"sum_length", cm:rint(rng,600,1300) },
    { type:"sell_count", amount:rint(rng,6,14) },
    { type:"earn_coin", coin:rint(rng,15000,50000) },
    { type:"get_junk", amount:rint(rng,2,5) },
    { type:"spend_coin", coin:rint(rng,6000,25000) },
    { type:"consume_bait", amount:rint(rng,10,30) },
    { type:"sequence_rarity", pattern:["노말","레어","유니크"], need:1 },
    { type:"sequence_rarity", pattern:["유니크","레어","노말"], need:1 },
...
function makeWeeklyDefs(wKey){
  ...
  const candidates = [
    ...
    { type:"catch_each_timeband", perBand:rint(rng,5,10) },
    { type:"get_junk", amount:rint(rng,10,20) },
    { type:"spend_coin", coin:rint(rng,100000,400000) },
    { type:"consume_bait", amount:rint(rng,80,200) },
    { type:"sequence_rarity", pattern:["노말","레어","유니크"], need:3 },
    { type:"sequence_rarity", pattern:["유니크","레어","노말"], need:3 },
    { type:"streak_junk", need:3 },
    { type:"streak_same_rarity", need:3 },
    { type:"catch_specific", species:sp, amount:rint(rng,5,10) },
    { type:"use_durability", items:rint(rng,2,4), ticks:rint(rng,20,60) },
    { type:"gear_unique_combo", rod:CONFIG.gear.rod, float:CONFIG.gear.float, rarity:"유니크", amount:3 }
  ];
  return rshuffle(rng,candidates).slice(0,5).map((def,i)=>attachId("weekly", wKey, def,i));
}
function attachId(scope, key, def, idx){ const raw = `${scope}|${key}|${JSON.stringify(def)}|${idx}`; def.id = `${scope}:${key}:${hash32(raw).toString(36)}`; def.scope = scope; return def; }
...
function rewardRoll(scope, tier, seedStr){
  ...
  if (scope==="daily"){ coin(3000,50000); if (rng()<0.80) be(5000,100000); if (rng()<0.70) bait(rng()<0.5?"지렁이 미끼":(rng()<0.5?"새우 미끼":"빛나는 젤리 미끼"), rint(rng,5,20)); if (rng()<0.07) key(1); }
  else { coin(50000,500000); be(100000,1500000); if (rng()<0.30) chest(1); else if (rng()<0.35) key(1); }
  return rewards;
}
...
function describeOne(def, st, tier){
  ...
  switch(def.type){
    ...
    case "sum_length": sub = pb(st.aux?.sumLen||0, def.cm); break;
    case "earn_coin": sub = pb(st.aux?.earned||0, def.coin); break;
    case "unique_species": sub = pb(Object.keys(st.aux?.uniq||{}).length, def.amount); break;
    case "big_length_once": sub = st.aux?.bigDone ? "완료됨" : `목표: ≥ ${def.cm}cm`; break;
    case "sequence_rarity": sub = `진행: ${st.aux?.seqCount||0}/${def.need}`; break;
    case "streak_junk": sub = `현재 연속: ${st.aux?.sj||0}/${def.need}`; break;
    case "streak_same_rarity": sub = `현재 연속: ${st.aux?.sr||0}/${def.need}`; break;
    case "use_durability": sub = `아이템 ${Object.keys(st.aux?.items||{}).length}/${def.items} • 내구도 ${st.aux?.ticks||0}/${def.ticks}`; break;
  }
  return { id:def.id, scope:def.scope, title: label(def), sub, rewardText, done: isDone(def, st), claimed: !!st.claimed };
}
function needOf(def){ switch(def.type){
  case "catch_any":
  case "catch_rarity_atleast":
  case "catch_timeband":
  case "sell_count":
  ...
  case "spend_coin":
  case "consume_bait":
  case "gear_unique_combo": return def.amount||def.coin||0;
} return 0; }
...
function isDone(def, st){
  switch(def.type){
    ...
    case "catch_each_timeband":{
      ...
      return ["낮","노을","밤"].every(b => (pb[b]||0) >= def.perBand);
    }
...
function onCatch(u, ctx){
  ...
  for (const def of [...dailyDefs(), ...weeklyDefs()]){
    ...
    switch(def.type){
      ...
      case "catch_rarity_atleast": if (RARITY_ORDER.indexOf(rarity) >= RARITY_ORDER.indexOf(def.rarity)) st.progress++; break;
      case "catch_timeband": if (band === def.band) st.progress++; break;
      case "catch_each_timeband": { const m=(st.aux.perBand ||= {}); if (["낮","노을","밤"].includes(band)) m[band]=(m[band]||0)+1; break; }
      case "catch_specific": if (!def.species || def.species===name) st.progress++; break;
      case "sum_length": st.aux.sumLen = (st.aux.sumLen||0) + len; break;
      case "unique_species": (st.aux.uniq ||= {})[name] = true; break;
      case "big_length_once": if (len >= def.cm) st.aux.bigDone = true; break;
      case "get_junk": if (isJunk) st.progress++; break;
      case "sequence_rarity": {
        if (isJunk) { st.aux.seqIdx=0; break; }
        const p = def.pattern; const idx = st.aux.seqIdx||0;
        if (rarity===p[idx]) { const nx = idx+1; if (nx===p.length){ st.aux.seqCount=(st.aux.seqCount||0)+1; st.aux.seqIdx=0; } else st.aux.seqIdx=nx; }
        else { st.aux.seqIdx = (rarity===p[0]? 1:0); }
        break;
      }
      case "streak_junk": { const cur=(st.aux.sj||0); st.aux.sj = isJunk ? cur+1 : 0; break; }
      case "streak_same_rarity": { const last=st.aux.lastR||null; if (last && rarity===last) st.aux.sr=(st.aux.sr||1)+1; else st.aux.sr=1; st.aux.lastR=rarity; break; }
      case "gear_unique_combo": { if (rarity===def.rarity && ctx.equip?.rod===CONFIG.gear.rod && ctx.equip?.float===CONFIG.gear.float) st.progress++; break; }
    }
  }
}
function onSell(u, soldCount, coinEarned){
  ...
  resetKeysIfNeeded(u, dKey, wKey);
  for (const def of [...dailyDefs(), ...weeklyDefs()]){
    const st = getState(u, def.id); if (st.claimed || isDone(def, st)) continue;
...
function onDurability(u, itemKey, ticks=1){
  ...
  for (const def of [...dailyDefs(), ...weeklyDefs()]){
    const st = getState(u, def.id); if (st.claimed || isDone(def, st)) continue;
    if (def.type==="use_durability"){ (st.aux.items ||= {})[itemKey] = true; st.aux.ticks = (st.aux.ticks||0) + Math.max(0,ticks); }
  }
}
...
function snapshot(u){
  ...
  const daily = makeDailyDefs(dKey).map(def => describeOne(def, getState(u,def.id), tier));
  const weekly = makeWeeklyDefs(wKey).map(def => describeOne(def, getState(u,def.id), tier));
  return { dailyKey:dKey, weeklyKey:wKey, daily, weekly };
}
function claim(u, scope, questId){
  const tier = u.tier || "실버";
  const defs = scope==="daily" ? makeDailyDefs(dayKey0900KST()) : makeWeeklyDefs(weekKeyMon0900KST());
...
function configure(opts={}){
  ...
  if (opts.gear){ if (opts.gear.rod) CONFIG.gear.rod = String(opts.gear.rod); if (opts.gear.float) CONFIG.gear.float = String(opts.gear.float); }
}

module.exports = { configure, snapshot, claim, onCatch, onSell, onOpenChest, onSpend, onBaitUse, onDurability };
