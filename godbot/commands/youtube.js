...
module.exports.handleComponent = async function(interaction) {
  try {
    ...
    if (!(cid.startsWith(SESS_PREFIX) || cid.startsWith(CH_SESS_PREFIX))) return;

    const isSearch = cid.startsWith(SESS_PREFIX);
    const token = isSearch ? cid.slice(SESS_PREFIX.length) : cid.slice(CH_SESS_PREFIX.length);
    const [op, sid] = token.split(":"); // prev|next

    const sess = sessions.get(sid);
    if (!sess) return interaction.reply({ content: "세션이 만료되었어. 다시 실행해줘!", ephemeral: true });
    if (interaction.user.id !== sess.owner) {
      return interaction.reply({ content: "요청자만 조작할 수 있어.", ephemeral: true });
    }
    if (Date.now() > sess.expireAt) {
      sessions.delete(sid);
      return interaction.reply({ content: "세션이 만료되었어. 다시 실행해줘!", ephemeral: true });
    }

    if (isSearch && sess.type === "search") {
      if (op === "prev") sess.index = Math.max(0, sess.index - 1);
      if (op === "next") sess.index = Math.min(sess.list.length - 1, sess.index + 1);

      const key = process.env.YT_API_KEY;
      const cur = sess.list[sess.index];
      const more = await ytVideoInfo(cur.id, key);
      if (!more) return interaction.deferUpdate().catch(()=>{});

      const { embed, url } = buildEmbedForVideo(more.video, more.channel, more.recentComment, sess.index, sess.list.length);
      const row = buildPagerRow(sid, sess.index, sess.list.length);

      if (sess.playerMsgId) {
        try {
          const ch = await interaction.client.channels.fetch(sess.channelId);
          const pmsg = await ch.messages.fetch(sess.playerMsgId);
          await pmsg.edit({ content: url, allowedMentions: { parse: [] } });
        } catch {}
      }
    ...
    console.error("[유튜브 handleComponent 예외]", e);
    if (!interaction.replied && !interaction.deferred) {
      await interaction.reply({ content: "❌ 처리 중 오류", ephemeral: true }).catch(()=>{});
    }
  }
};
