...
function evalHand(cards) {
  ...
  if (vcnt[0][1] === 4) return { rank:7, value:[+vcnt[0][0]] };
  if (vcnt[0][1] === 3 && vcnt[1] && vcnt[1][1] >=2) return { rank:6, value:[+vcnt[0][0], +vcnt[1][0]] };
  if (flush) return { rank:5, value:flush.slice(0,5) };
  let st = findStraight(valArr);
  if (st) return { rank:4, value:st };
  if (vcnt[0][1] === 3) return { rank:3, value:[+vcnt[0][0]] };
  if (vcnt[0][1] === 2 && vcnt[1] && vcnt[1][1] === 2) return { rank:2, value:[+vcnt[0][0], +vcnt[1][0]] };
  if (vcnt[0][1] === 2) return { rank:1, value:[+vcnt[0][0]] };
  return { rank:0, value:valArr.slice(0,5) };
}
  ...
  async execute(interaction) {
    ...
    if (sub === "개설") {
      ...
      const row = new ActionRowBuilder().addComponents(
        ...
        new ButtonBuilder().setCustomId("poker_start").setLabel("모집 마감/게임 시작").setStyle(ButtonStyle.Primary)
      );
      const embed = new EmbedBuilder()
        .setTitle("🃏 포커방 모집")
        .setDescription(`텍사스 홀덤 포커를 시작합니다!\n\n- ${max}명까지 참가 가능\n- 참가비: **${entry} BE**\n- 블라인드: SB ${sb} / BB ${bb}\n\n방장이 [모집 마감/게임 시작] 누르면 게임 시작!`)
      ...
      collector.on('collect', async btnInt => {
        ...
        // 모집 마감/시작
        if (btnInt.customId === "poker_start") {
          if (btnInt.user.id !== room.hostId) return btnInt.reply({ content:"방장만 시작할 수 있습니다.", ephemeral:true });
          if (room.players.length < 2) return btnInt.reply({ content:"2명 이상 참가해야 시작할 수 있습니다.", ephemeral:true });
          if (room.state !== "WAIT") return btnInt.reply({ content:"이미 시작됨", ephemeral:true });
          room.state = "DEAL";
          collector.stop();
          await btnInt.reply({ content:"모집을 마감하고 게임을 시작합니다.", ephemeral:true });
