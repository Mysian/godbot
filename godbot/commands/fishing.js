...
async function autoBuyOne(u, db, kind, name) {
  ...
  if (kind === "bait") {
    ...
    if (kind === "rod") { addRod(u, name); if (name === "전설의 낚싯대") recordFirst(db, "legendRod", u._uid); }
...
function rankButtons(mode){
  return new ActionRowBuilder().addComponents(
    ...
    , new ButtonBuilder().setCustomId("rank:firsts").setLabel("최초").setStyle(mode==="firsts"?ButtonStyle.Primary:ButtonStyle.Secondary)
  );
}
...
const DROP_TABLE = {
  "노말":   ["멸치","피라냐","금붕어","작은 새우","빈 페트병","해초","뚱이의 바지","갓봇의 안경","낚시 코인","작은입배스","홍어","가오리","우럭","민어","병어","방어","전어","은어","송어","넙치","청어","꽁치",
            "쏘가리","농어","뼈 생선","피라미","해마","앵무조개","따분한 멸치","등푸른 생선","모래 송사리"],
  "레어":   ["전갱이","고등어","가재","연어","다랑어","가자미","오징어","잉어","삼치","복어","황어","도미","참돔","붕어","비단 잉어","빙어","갈치","파랑 정수","큰입배스","참다랑어","황다랑어",
             "꼴뚜기","쏠배감펭","개구리","홍게","별점어","돌꼬치","붉은점 돌돔","파도 송사리","푸른 바다뱀","푸른 복어"],
