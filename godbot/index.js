...
const botManage = require('./commands/bot.js');
...
if (interaction.customId.startsWith('profile:')) return;
  ...
  if (interaction.customId.endsWith('_open')) {
    ...
    if (interaction.customId === 'bot_pull_open')             return await botManage.runGitPull(interaction);
    if (interaction.customId === 'bot_deploy_commands_open')  return await botManage.runDeployCommands(interaction);
    if (interaction.customId === 'bot_restart_open')          return await botManage.runPm2Restart(interaction);
